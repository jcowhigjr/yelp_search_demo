  ...
  test 'Links to all available locales will show' do
    [:en, :"pt-BR"].each do |locale|
      I18n.with_locale(locale) do
        ...
        # Verify language selector links exist in footer navigation
        # Default locale link (English) should point to root
        assert_select "footer .language-nav a[href='/']", minimum: 1
        ...
        locales_except_default.each do |l|
          # Language links should be in footer .language-nav
          assert_select "footer .language-nav a[href='/#{l}']", minimum: 1
        end
      end
    end
  end
