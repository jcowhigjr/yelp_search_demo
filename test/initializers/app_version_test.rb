...
class AppVersionConfigTest < ActiveSupport::TestCase
  def test_app_version_matches_version_constant
    assert_equal Jitter::VERSION.next, Rails.configuration.x.app_version
  end
  ...
  def test_version_constant_matches_version_file
    ...
    assert_equal Jitter::VERSION.next, version_file_content, 'Jitter::VERSION should match VERSION file content'
  end
end